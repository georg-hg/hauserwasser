...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (dayRows.length === 0) return res.status(404).json({ error: 'Fischtag nicht gefunden.' });
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!fishingDate) return res.status(400).json({ error: 'Datum ist ein Pflichtfeld.' });
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      UPDATE fishing_days SET technique = $1, notes = $2
      WHERE id = $3 AND user_id = $4 RETURNING *
    `, [technique || null, notes || null, req.params.id, req.user.id]);
    if (rows.length === 0) return res.status(404).json({ error: 'Fischtag nicht gefunden.' });
    res.json(rows[0]);
  } catch (err) {
...
router.put('/:id/complete', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      UPDATE fishing_days SET completed = true,
        notes = COALESCE($1, notes), technique = COALESCE($2, technique)
      WHERE id = $3 AND user_id = $4 RETURNING *
...
router.put('/:id/position', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      UPDATE fishing_days SET latitude = $1, longitude = $2, position_updated_at = NOW()
    ...
    if (rows.length === 0) return res.status(404).json({ error: 'Aktiver Fischtag nicht gefunden.' });
    res.json(rows[0]);
  } catch (err) {
...
router.delete('/:id', auth, async (req, res) => {
  try {
    await pool.query('DELETE FROM catches WHERE fishing_day_id = $1 AND user_id = $2', [req.params.id, req.user.id]);
    const { rowCount } = await pool.query(
      'DELETE FROM fishing_days WHERE id = $1 AND user_id = $2',
      [req.params.id, req.user.id]
    );
    if (rowCount === 0) return res.status(404).json({ error: 'Fischtag nicht gefunden.' });
    res.json({ message: 'Fischtag geloescht.' });
  } catch (err) {
    console.error('DELETE /fishing-days/:id error:', err);
    res.status(500).json({ error: 'Fischtag konnte nicht geloescht werden.' });
  }
});
...
router.post('/:id/catches', auth, upload.single('photo'), async (req, res) => {
  try {
    ...
    if (dayRows.length === 0) return res.status(404).json({ error: 'Fischtag nicht gefunden.' });
    const day = dayRows[0];
    const { catchTime, latitude, longitude, locationName, fishSpecies, lengthCm, weightKg, kept, notes } = req.body;
    if (!fishSpecies) return res.status(400).json({ error: 'Fischart ist ein Pflichtfeld.' });
